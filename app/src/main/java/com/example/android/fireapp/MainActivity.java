...
import com.example.android.fireapp.menuButtons.EditData;
...
public class MainActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (sig == 0) {
           ...
           Signal.setText("Intermediate");
        } else if (sig == 3) {
           Signal.setText("Good");
        } else if (sig == 4) {
           Signal.setText("Great");
        }
                    ...
                    for (DataSnapshot child : dataSnapshot.getChildren()) {
                        if (child.getKey().equalsIgnoreCase(type)) {
                            for (DataSnapshot child2 : child.child("Signal").getChildren()) {
                                int count = 0;
                                int signal = Integer.parseInt(child2.getKey());
                                Log.i(MainActivity.class.getSimpleName(), "LinaLina  " + signal + "\n");
                                for (DataSnapshot child3 : child2.getChildren()) {
                                    ArrayList<LatLng> newArray = new ArrayList<>();
                                    for (DataSnapshot child4 : child3.child("Data").getChildren()) {
                                        int area = Integer.parseInt(child3.getKey());
                                        int childNum = 0;
                                        Log.i(MainActivity.class.getSimpleName(), "LinaLinaArea : " + area + " ww " + signal + "\n");
                                        try {
                                            int counter = Integer.parseInt(child4.getKey());
                                            childNum = counter;
                                            double latitude = child4.child("Latitude").getValue(Double.class);
                                            double longitude = child4.child("Longitude").getValue(Double.class);
                                            LatLng location = new LatLng(latitude, longitude);
                                            Log.i(MainActivity.class.getSimpleName(), "LinaLinaArea : " + area + " ww " + counter + " ww " + signal + " ww " + location + "\n");
                                            newArray.add(location);
                                            newArray.add(location);
                                        } catch (Exception e) {
                                            Log.i(MainActivity.class.getSimpleName(),"Linaaaaaaaaaaaa : "+signal+"\n"+area+"\n"+childNum+"\n");
                                            networkTypeReference.child(type).child("Signal").child(String.valueOf(signal)).child(String.valueOf(area)).child("Data").child(String.valueOf((childNum))).setValue(null);
                                        }
